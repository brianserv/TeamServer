...
void  CBackendClient::StartClient(char* pSrvIP, short nSrvPort)
{
	if (!InitParams(pSrvIP, nSrvPort))
		return;
	if (!m_workThreads.CreateThreads(1, (void*)CBackendClient::WorkingThreadsProc, this))
		return;
}

void  CBackendClient::StopClient()
{
	StopThread();
}

bool CBackendClient::InitParams(char* pSrvIP, short nSrvPort)
{
	bool bFlag = true;
	m_bISSuccConnect = false;
...
int CBackendClient::InitTcpSocket()
{
...
bool CBackendClient::ConnectSrv()
{
	bool          bFlag = false;
	int           nReValue = 0;
...
void CBackendClient::DisConnectSrv()
{
	m_bISSuccConnect = false;
	m_bIsRegister = false;
	m_TCPSocket.Close();
...
bool CBackendClient::CheckISSuccConnect()
{
	if (!m_bISSuccConnect)
	{
		if (!ConnectSrv())
...
DWORD CBackendClient::WorkingThreadsProc(LPVOID lpParam)
{
	CBackendClient* pThis = (CBackendClient*)lpParam;
	if (pThis)
	{
		pThis->WorkingProc();
	}
...
#else
void*  CBackendClient::WorkingThreadsProc(void* Param)
{
	CBackendClient* pThis = (CBackendClient*)Param;
	if (pThis)
	{
		pThis->WorkingProc();
	}
...
void CBackendClient::WorkingProc()
{
	if (InitTcpSocket())
		return;
	try
...
void CBackendClient::StopThread()
{
	m_bStopThread = true;
}

void CBackendClient::ReleaseResource()
{
	DisConnectSrv();
	//
	if (NULL != m_pRecvDataBuffer)
...
void CBackendClient::SendData(char* pSendDataInfo, long nSendDataLen)
{
	int     nReValue = 0;
...
void CBackendClient::RecvAllData()
{
...
void CBackendClient::ProcessClientMsgInfo(char*   pDataInfo, long nDataLen, char* pSourceIP, short nSourcePort)
{
...
void CBackendClient::Process_HeartBeatReq()
{
...
void CBackendClient::Process_HeartBeatRes(char* pDataInfo, long nDataLen)
{
...
void CBackendClient::Process_RoleDisconnectReq(int nRoleId)
{
	char sendbuf[SERVER_MESSAGE_LEN_MAX] = { 0 };
	int sendlen = 0;
...
void CBackendClient::Process_GirlDisconnectReq(int nGirlID)
{
	char sendbuf[SERVER_MESSAGE_LEN_MAX] = { 0 };
	int sendlen = 0;

...
bool  CConfigFile::ReadConfigFile()
{
	...
	//#LogLevel
	m_nLogLevel = EQ_ReadInifileInt("Setting", "LogLevel", nDefaultConfigValue, "HallServer.ini");

	//#LocalIp
	memset(m_szLocalIp, 0, sizeof(m_szLocalIp));
	EQ_ReadInifileString("Setting", "LocalIp", m_szLocalIp, sizeof(m_szLocalIp), szDefaultConfigValue, "HallServer.ini");

	//#Listen LocalPort
	m_nLocalPort = EQ_ReadInifileInt("Setting", "LocalPort", nDefaultConfigValue, "HallServer.ini");

	//#Connected GS_1
	memset(m_szGameServerAddress, 0, sizeof(m_szGameServerAddress));
	EQ_ReadInifileString("Setting", "GameServerAddress", m_szGameServerAddress, sizeof(m_szGameServerAddress), szDefaultConfigValue, "HallServer.ini");

	//#DataBaseLink Info
	memset(m_szDBIp, 0, sizeof(m_szDBIp));
	EQ_ReadInifileString("Setting", "DBIp", m_szDBIp, sizeof(m_szDBIp), szDefaultConfigValue, "HallServer.ini");

	m_nDBPort = EQ_ReadInifileInt("Setting", "DBPort", nDefaultConfigValue, "HallServer.ini");

	memset(m_szDBName, 0, sizeof(m_szDBName));
	EQ_ReadInifileString("Setting", "DBName", m_szDBName, sizeof(m_szDBName), szDefaultConfigValue, "HallServer.ini");

	memset(m_szUserName, 0, sizeof(m_szUserName));
	EQ_ReadInifileString("Setting", "UserName", m_szUserName, sizeof(m_szUserName), szDefaultConfigValue, "HallServer.ini");

	memset(m_szPassword, 0, sizeof(m_szPassword));
	EQ_ReadInifileString("Setting", "Password", m_szPassword, sizeof(m_szPassword), szDefaultConfigValue, "HallServer.ini");
...
bool  CConfigFile::CheckConfigFile()
{
	...
	EQ_SPRINTF(szLogDir_IniConfig, sizeof(szLogDir_IniConfig), "TeamHallServerLog/TeamHall_INIConfig(%d.%d.%d.%d.%d.%d)", CEQ_Time::Instance().EQ_GetYear(), CEQ_Time::Instance().EQ_GetMonth(),
		CEQ_Time::Instance().EQ_GetDay(), CEQ_Time::Instance().EQ_GetHour(), CEQ_Time::Instance().EQ_GetMin(),
		CEQ_Time::Instance().EQ_GetSec());

	g_LogFile.InitLogFile(szLogDir_IniConfig, "TeamHall_INIConfig", 100);

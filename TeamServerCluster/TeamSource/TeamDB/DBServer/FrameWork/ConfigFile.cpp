...
bool  CConfigFile::ReadConfigFile()
{
	...
	//#LogLevel
	m_nLogLevel = EQ_ReadInifileInt("Setting", "LogLevel", nDefaultConfigValue, "DBServer.ini");

	//#Listen LocalPort
	m_nLocalPort = EQ_ReadInifileInt("Setting", "LocalPort", nDefaultConfigValue, "DBServer.ini");

	//#LocalIP
	memset(m_szLocalIp, 0, sizeof(m_szLocalIp));
	EQ_ReadInifileString("Setting", "LocalIp", m_szLocalIp, sizeof(m_szLocalIp), szDefaultConfigValue, "DBServer.ini");

	//#DataBaseLink Info
	memset(m_szDBIp, 0, sizeof(m_szDBIp));
	EQ_ReadInifileString("Setting", "DBIp", m_szDBIp, sizeof(m_szDBIp), szDefaultConfigValue, "DBServer.ini");

	m_nDBPort = EQ_ReadInifileInt("Setting", "DBPort", nDefaultConfigValue, "DBServer.ini");

	memset(m_szDBName, 0, sizeof(m_szDBName));
	EQ_ReadInifileString("Setting", "DBName", m_szDBName, sizeof(m_szDBName), szDefaultConfigValue, "DBServer.ini");

	memset(m_szUserName, 0, sizeof(m_szUserName));
	EQ_ReadInifileString("Setting", "UserName", m_szUserName, sizeof(m_szUserName), szDefaultConfigValue, "DBServer.ini");

	memset(m_szPassword, 0, sizeof(m_szPassword));
	EQ_ReadInifileString("Setting", "Password", m_szPassword, sizeof(m_szPassword), szDefaultConfigValue, "DBServer.ini");
...
bool  CConfigFile::CheckConfigFile()
{
	...
	EQ_SPRINTF(szLogDir_IniConfig, sizeof(szLogDir_IniConfig), "TeamDBServerLog/TeamDBServer_INIConfig(%d.%d.%d.%d.%d.%d)", CEQ_Time::Instance().EQ_GetYear(), CEQ_Time::Instance().EQ_GetMonth(),
		CEQ_Time::Instance().EQ_GetDay(), CEQ_Time::Instance().EQ_GetHour(), CEQ_Time::Instance().EQ_GetMin(),
		CEQ_Time::Instance().EQ_GetSec());

	g_LogFile.InitLogFile(szLogDir_IniConfig, "TeamDBServer_INIConfig", 100);

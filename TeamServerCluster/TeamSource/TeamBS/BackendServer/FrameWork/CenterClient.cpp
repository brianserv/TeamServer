...
void CCenterClient::ProcessClientMsgInfo(char*   pDataInfo, long nDataLen, char* pSourceIP, short nSourcePort)
{
	...
	try
	{
		...
		case ESGT_PING_S2GT_HEARTBEAT_RES:
			...
			break;
		default:
			CBackendServer::Instance().SendClientMsg(pDataInfo, nDataLen, pPacketHead->GetIndex());
			break;
		}
	}
	catch (...)
...
void CCenterClient::Process_HeartBeatReq()
{
	...
	g_CenterClient.SendData(sendbuf, sendlen);
}
...
void CCenterClient::Process_RoleDisconnectReq(int nRoleId)
{
	char sendbuf[SERVER_MESSAGE_LEN_MAX] = { 0 };
	int sendlen = 0;
...
void CCenterClient::Process_GirlDisconnectReq(int nGirlID)
{
	char sendbuf[SERVER_MESSAGE_LEN_MAX] = { 0 };
	int sendlen = 0;
